...
var lines = String(contents).split(/\n/);
 ...
 for(var i =1; i < lines.length -1;i++){
    ...
    'uri': href,
    'headers':{
        'cookie': 'AHrlqAAAAAMADnyJxzQ7oI4ALtotww=='
    } 
}
    //console.log(href);
                        ...
                        request({uri: "https://www.lafourchette.com"+String(link)}, function(error, response, body)
                                  {
                                    ...
                                    $(".saleType--specialOffer").each(function()
                                    {
                                        ...
                                        if(deal != "")
                                        {
                                          json.deal = deal;
                                          //console.log(deal);
                                           fs.appendFile('outputDeals.json', JSON.stringify(json)+"\r\n", function(err){ });
                                        }
                                        else{
                                            console.log("Pas de deal pour" + title)
                                        }
                                        });
                                    });
                        }
                        else{
                            console.log("Pas de restaurant pour ce lien : "+config.uri)
                        }
                     });
                 });
  }